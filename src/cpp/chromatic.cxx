...
namespace {
    ...
    tuple<vector<T>, vector<index_t>> sort_with_indices(const vector<T>& v, bool (*compare)(const T& a, const T& b)) {
        ...
        std::stable_sort(idx.begin(), idx.end(),
            [&v, &compare](index_t i1, index_t i2) { return compare(v[i1], v[i2]); });
...
namespace chalc {
    ...
    FilteredComplex chromatic_delrips_complex(const RealMatrix<double>& points, const vector<index_t>& colours) {
        ...
        if (delX.max_dim >= 1) {
            for (int p = delX.max_dim; p >= 1; p--) {
                for (auto& [idx, simplex] : delX.get_simplices()[p]) {
                    ...
                    auto [centre, sqRadius, success] = cmb::constrained_miniball<double>(points.rows(), points(all, verts), E, b);
                    numerical_issues |= !success;
                    // check if the stack is empty
                    bool stack_is_empty = true;
                    for (auto& [j, verts_j] : verts_by_colour_in_simplex) {
                        // radius of sphere of colour j
                        double rj = (points(all, verts_j).colwise() - centre).colwise().squaredNorm().minCoeff();
                        double distance_to_nearest_point_of_colour_j = (points(all, verts_by_colour[j]).colwise() - centre).colwise().squaredNorm().minCoeff();
    ...
    FilteredComplex chromatic_delcech_complex(const RealMatrix<double>& points, const vector<index_t>& colours) {
        ...
        if (delX.max_dim >= 1) {
            for (int p = delX.max_dim; p >= 1; p--) {
                for (auto& [idx, simplex] : delX.get_simplices()[p]) {
                    auto verts = simplex->get_vertex_labels();
                    auto [centre, sqRadius, success] = cmb::miniball<double>(points.rows(), points(all, verts));
                    numerical_issues |= !success;
                    simplex->value = sqrt(sqRadius);
                }
            }            
