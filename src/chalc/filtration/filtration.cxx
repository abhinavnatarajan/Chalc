...
using std::sort, std::stable_sort, std::min, std::max, std::fill,
    std::adjacent_find, std::prev_permutation, std::invalid_argument,
    std::runtime_error, std::bad_weak_ptr;
} // namespace
...
shared_ptr<FilteredComplex::Simplex> FilteredComplex::Simplex::make_Simplex(
    index_t label, index_t max_vertex, value_t value,
    const vector<shared_ptr<Simplex>> &facets) {
    auto self =
        shared_ptr<Simplex>(new Simplex(label, max_vertex, value, facets));
...
FilteredComplex::Simplex::Simplex(index_t label, index_t max_vertex,
                                  value_t value,
                                  const vector<shared_ptr<Simplex>> &facets)
    : label(label), max_vertex(max_vertex),
      dim(facets.size() == 0 ? 0 : facets.size() - 1), value(value),
      facets(facets) {}
...
inline void FilteredComplex::Simplex::add_colour(index_t c) { colours.set(c); }

inline void FilteredComplex::Simplex::add_colours(colours_t c) { colours |= c; }
...
inline void FilteredComplex::Simplex::make_colourless() { colours.reset(); }

void FilteredComplex::Simplex::set_colour(index_t c) { colours.reset().set(c); }

colours_t FilteredComplex::Simplex::get_colours() { return colours; }

unsigned long long int FilteredComplex::Simplex::get_colours_as_int() {
    return colours.to_ullong();
}

FilteredComplex::FilteredComplex(const index_t num_vertices,
                                 const index_t max_dimension)
    : binomial(make_shared<BinomialCoeffTable>(
          num_vertices,
          max_dimension +
              1)), // we want nCk for all 0 <= n <= num_vertices and 0 <= k <=
                   // max_num_verts_in_a_simplex = max_dim + 1
      simplices(max_dimension + 1), N(num_vertices), max_dim(max_dimension),
      num_simplices(num_vertices), cur_dim(0) {
    if (max_dim < 0) {
...
void FilteredComplex::check_vertex_sequence_is_valid(
    vector<index_t> &verts) const {
...
index_t FilteredComplex::_get_label_from_vertex_labels(
    const vector<index_t> &verts) const {
    index_t label = 0;
...
index_t
FilteredComplex::get_label_from_vertex_labels(vector<index_t> &verts) const {
    check_vertex_sequence_is_valid(verts);
    return _get_label_from_vertex_labels(verts);
}

bool FilteredComplex::_has_simplex(const index_t dim,
                                   const index_t label) const {
...
bool FilteredComplex::has_simplex(const index_t dim,
                                  const index_t label) const {
...
FilteredComplex::_add_simplex(const vector<index_t> &verts,
    ...
    if (search_simplex != simplices[dim].end()) {
        ...
        new_simplex->value =
            min(max(filt_value, static_cast<value_t>(0)), new_simplex->value);
    } else {
        ...
        new_simplex = Simplex::make_Simplex(label, max_vertex, filt_value,
                                            std::move(facets));
...
void FilteredComplex::propagate_filt_values(const index_t start_dim,
                                            const bool up) {
...
index_t FilteredComplex::dimension() const noexcept { return cur_dim; }
...
FilteredComplex::serialised() const {
    ...
    for (index_t d = 0, i = 0; d <= cur_dim; d++) {
        ...
        stable_sort(
            sort_by_val.begin(), sort_by_val.end(),
...
FilteredComplex FilteredComplex::clique_complex(const index_t n,
                                                const index_t k) {
    if (n <= 0) {
