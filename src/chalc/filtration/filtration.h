...
struct FilteredComplex {
	...
	std::vector<std::tuple<std::vector<index_t>, index_t, value_t,
...
private:
	...
	index_t
	_get_label_from_vertex_labels(const std::vector<index_t> &verts) const;
...
struct FilteredComplex::Simplex
	: public std::enable_shared_from_this<FilteredComplex::Simplex> {
	...
	make_Simplex(index_t label, index_t max_vertex,
...
private:
	...
	std::vector<std::weak_ptr<Simplex>>
		cofacets;	   // pointers to the cofacets of the simplex
	colours_t colours; // bitmask representing the colours of its vertices
	// Constructor
	Simplex(index_t label, index_t max_vertex, value_t value,
			const std::vector<std::shared_ptr<Simplex>> &facets);
