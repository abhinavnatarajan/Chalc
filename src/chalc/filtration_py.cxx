...
PYBIND11_MODULE(filtration, m)
{
	...
	filtered_complex.def(py::init<const index_t, const index_t>(),
		...
		.def("has_simplex",
			static_cast<bool (FilteredComplex::*)(vector<index_t>& v) const >(&FilteredComplex::has_simplex),
			R"docstring(
	...
	simplex.def_readonly("dimension", &FilteredComplex::Simplex::dim,
		R"docstring(
		...
		static_cast<vector<index_t> (FilteredComplex::Simplex::*)() const >(&FilteredComplex::Simplex::get_vertex_labels),
			R"docstring(
		...
		.def_property_readonly("colours", &FilteredComplex::Simplex::get_colours_as_int,
			R"docstring(
		...
		.def("set_colour", [](const shared_ptr<FilteredComplex::Simplex> &s_ptr, index_t c)
			{
				if (s_ptr->dim == 0)
				{
			...
			py::arg("colour"))
		.def_property_readonly("facets", &FilteredComplex::Simplex::get_facets,
			R"docstring(
