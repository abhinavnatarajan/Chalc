...
using chalc::index_t, chalc::MAX_NUM_COLOURS, Eigen::lastN, Eigen::all,
	std::min, std::runtime_error, std::bad_weak_ptr, std::stable_sort;
...
using Triangulation_data_structure = CGAL::Triangulation_data_structure<
	Kernel_d::Dimension, CGAL::Triangulation_vertex<Kernel_d, size_t>,
	CGAL::Triangulation_full_cell<Kernel_d>>;
using DelaunayTriangulation =
	CGAL::Delaunay_triangulation<Kernel_d, Triangulation_data_structure>;
...
tuple<RealMatrix<Real_t>, RealVector<Real_t>>
...
*/
RealMatrix<double> stratify(const RealMatrix<double> &points,
...
FilteredComplex delaunay(const RealMatrix<double> &X,
						 const vector<index_t> &colours) {
	optional<RealMatrix<double>> Y{};
	try {
		Y = stratify(X, colours);
	} catch (...) {
		throw;
	}
	auto dim = Y->rows();
	...
	if (Y->cols() != 0) {
		...
		auto [sorted_points, sorted_indices] = sort_with_indices<Point_d>(
			points,
			[](const Point_d &a, const Point_d &b) -> bool { return a < b; });
		...
		// original label
		for (auto vert_it = delY.finite_vertices_begin();
			 vert_it != delY.finite_vertices_end(); vert_it++) {
			// find the index of its associated point
			auto point = vert_it->point();
			const Point_d *p = static_cast<Point_d *>(
				std::bsearch(&point, sorted_points.data(), sorted_points.size(),
							 sizeof(Point_d), compare<Point_d>));
		...
		for (auto cell_it = delY.finite_full_cells_begin();
			 cell_it != delY.finite_full_cells_end(); cell_it++) {
			// iterate over the vertices of the cell and get their labels
			for (auto [label_it, vert_it] =
					 tuple{max_cell_vertex_labels.begin(),
						   cell_it->vertices_begin()};
				 vert_it != cell_it->vertices_end(); label_it++, vert_it++) {
...
FilteredComplex delrips(const RealMatrix<double> &points,
						const vector<index_t> &colours) {
	// Start
	// Get the delaunay triangulation
	optional<FilteredComplex> delX{};
	try {
		delX = delaunay(points, colours);
	} catch (...) {
		throw;
	}
	...
	if (delX->dimension() >= 1) {
		for (auto &[idx, edge] : delX->get_simplices()[1]) {
			...
			edge->value =
				(points.col(verts[0]) - points.col(verts[1])).norm() * 0.5;
		}
...
FilteredComplex alpha(const RealMatrix<double> &points,
					  const vector<index_t> &colours, std::ostream &ostream) {
	// Start
	// Get the delaunay triangulation
	optional<FilteredComplex> delX{};
	try {
	...
	if (delX->dimension() >= 1) {
		...
		*/
				...
				for (auto &[j, verts_j] : verts_by_colour_in_simplex) {
					...
					E.conservativeResize(E.rows() + num_new_rows,
										 Eigen::NoChange);
					b.conservativeResize(b.rows() + num_new_rows);
					RealMatrix<mpreal>::BlockXpr E_new_rows =
						E.bottomRows(num_new_rows);
					Eigen::VectorBlock<RealVector<mpreal>> b_new_rows =
		...
		*/
				auto [centre, sqRadius, success] =
					cmb::constrained_miniball<mpreal>(X(all, verts), E, b);
		  ...
		  */
					mpreal rj_squared = (X(all, verts_j).colwise() - centre)
											.colwise()
											.squaredNorm()
											.minCoeff();
					mpreal squared_dist_to_nearest_pt_of_colour_j =
						(X(all, verts_by_colour[j]).colwise() - centre)
							.colwise()
							.squaredNorm()
							.minCoeff();
	...
	if (numerical_instability) {
		ostream << "Warning: encountered numerical problems. Filtration values "
				   "may be inaccurate."
				<< std::endl;
	}
	return *delX;
...
FilteredComplex delcech(const RealMatrix<double> &points,
						const vector<index_t> &colours, std::ostream &ostream) {
	// Start
	// Get the delaunay triangulation
	optional<FilteredComplex> delX{};
	...
	if (delX->dimension() >= 1) {
		...
		for (auto &[idx, edge] : delX->get_simplices()[1]) {
			...
			edge->value = static_cast<double>(
							  (X.col(verts[0]) - X.col(verts[1])).norm()) *
						  0.5;
